...
contract OneTMShowOff is ERC721Enumerable, Ownable {
    ...
    function claim(bytes32[] calldata _merkleProof, uint256 _amount) external {
        ...
        require(
            MerkleProof.verify(_merkleProof, claimMerkleRoot, leaf),
            "Proof Invalid"
        );
    ...
    function presale(bytes32[] calldata _merkleProof, uint256 _amount)
        external
        payable
    {
        uint256 supply = totalSupply();
    ...
    function mint(bytes32[] calldata _merkleProof, uint256 _amount)
        external
        payable
    {
        uint256 supply = totalSupply();
